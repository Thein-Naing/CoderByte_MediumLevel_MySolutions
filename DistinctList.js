...
function DistinctList(arr) { 
  ...
  // code goes here  
  return arr.filter((num, index) => arr.indexOf(num) !== index).length;      /* 1. Use arr.filter() method together with arr.indexOf() method and arr.length method and return it.
}                                                                                  if arr.indexOf(0) ===  0  !== [0,-2,-2,5,5,5][0], then there will be duplicate entries and
                                                                                   filter this element and find its length.   */
