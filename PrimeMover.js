...
/*3.run for loop  until the Math.sqrt(n). non-prime number will definitely be less than or equal to sqrt(n).
  ...
  const isPrime = (n) => {  
   ...
   return true; // if iterate over all(n) & n will be prime if it was not divisible by any number(i).
  }
...
e.g. [3,5,7,9,11,13,17,19,23] ===> 9th position of prime number is 23. (num = 9 , n = 23).
    ...
    while (count < num ) { //  use while loop . initially count = 0 and so count < num;
     if (isPrime(n)) {
  ...
  //5. otherwise update prime number by decreasing the n by 1 and return it.
   return n - 1; // 
  }
